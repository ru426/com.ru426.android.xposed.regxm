package com.ru426.android.xposed.regxm.tools;
...
public class ToolbarBrightnessController extends LinearLayout implements android.widget.CompoundButton.OnCheckedChangeListener, android.widget.SeekBar.OnSeekBarChangeListener{
	...
	public void initOption(){
		try {
			Context regxmContext = mContext.createPackageContext(com.ru426.android.xposed.regxm.Settings.PACKAGE_NAME, 3);
	...
	private void reDrawIcon(ImageView icon, int state){
		...
		if(state <= 0) return;
		try{					
			mDrawable.setColorFilter(XModUtil.getAccentColor(mContext), Mode.SRC_ATOP);
		}catch(NotFoundException e){
